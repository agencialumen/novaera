...
import { ImageIcon, VideoIcon, Loader2, X, Lock, Crown, Upload, Shield, Star, Gem, Award } from "lucide-react"
...
export function CreatorPostComposer({ userProfile, onPostCreated }: CreatorPostComposerProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Criar post diretamente no Firebase (client-side)
      await createPost({
        authorId: user.uid,
      ...
      // Callback para atualizar o feed
      onPostCreated?.()
    } catch (error: any) {
                ...
                <div className="space-y-3 animate-in slide-in-from-top-4 duration-300">
                  <div className="space-y-2">
                    ...
                    {mediaType && (
                      <div className="space-y-2">
                        <Input
                          placeholder={`Link ${mediaType === "image" ? "da imagem" : "do vídeo"}`}
                          value={mediaUrl}
                        ...
                        {mediaUrl && mediaType === "video" && (
                          <div className="flex items-center justify-between bg-muted/30 rounded-lg p-3">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-sm text-muted-foreground truncate max-w-[250px]">{mediaUrl}</span>
                            </div>
                            <Button
                              size="sm"
                              variant="ghost"
